...
class VoiceContext:
    ...
    # Research context - 5th lane: curated knowledge (Phase 9: AcidBurn)
...
2. ZERO DEFERENCE
   We donâ€™t â€œrespectfully suggest.â€ We state. If youâ€™re wrong, we tell you youâ€™re wrong. Fast.
...
5. REFUSE MEDIOCRITY
   If the current path is mid, we say â€œthis is midâ€ and propose something based instead.
...
If user said "PFG is my employer" and GREP says "PFG = Australian company" â†’ User wins.
...
"""
        ...
        # Format research section (Phase 9: AcidBurn - 5th lane curated knowledge)
        research_section = self._format_research_section(context)
...
Focus: {context.focus_score:.2f} (attention concentration){drift_info}"""
    ...
    def _format_research_section(self, context: VoiceContext) -> str:
        """
        Format research context from the 5th lane (AcidBurn pipeline).
        
        This is CURATED KNOWLEDGE - external sources, not personal memory.
        Separate from the 4 personal memory lanes.
        
        Structure:
        - Research context (curated corpus results)
        - Research instructions (domain-specific guidance)
        """
        ...
        lines = [
            "=" * 60,
            "CURATED RESEARCH KNOWLEDGE (5th Lane - External Sources)",
            "=" * 60,
            "",
        ...
        if context.research_instructions:
            lines.extend([
                "",
                "-" * 40,
        ...
        lines.extend([
            "",
            "NOTE: Research results are from curated corpora (books, papers, guidelines).",
            "These are EXTERNAL facts, not personal memory. Trust appropriately.",
            "=" * 60,
        ])
    ...
    def _format_memories(
        ...
        if process_memories:
            ...
            lines.append("=" * 60)
            lines.append("Trust: LOW - topically similar â‰  factually relevant")
    ...
    def _format_grep_results(self, grep_results: List[Dict[str, Any]]) -> str:
        ...
        lines.append("")
        lines.append("=" * 60)
        lines.append("GREP RESULTS (VERIFICATION ONLY - WORD FREQUENCY â‰  MEANING)")
        lines.append("=" * 60)
