...
# Research pipeline - Phase 9: AcidBurn (5th lane)
from research_pipeline import ResearchPipeline, ResearchRetrievalResult
...
class CogTwin:
    ...
    def __init__(
        ...
        # Initialize research pipeline - Phase 9: AcidBurn (5th lane)
        self.research_pipeline = ResearchPipeline(
            data_dir=self.data_dir,
            embedder=self.retriever.embedder,
            web_search_fn=None,  # TODO: Wire web search for non-medical domains
        )
    ...
    async def think(
        ...
        # ===== STEP 2.6: Research Pipeline (5th Lane - AcidBurn) =====
        # Curated knowledge retrieval with domain-aware policies
        research_result: Optional[ResearchRetrievalResult] = None
        research_context = ""
        research_instructions = ""
        
        if cfg("research_pipeline.enabled", True):
            try:
                research_result = await self.research_pipeline.retrieve(
                    query=user_input,
                    query_embedding=query_embedding,
                    top_k=cfg("research_pipeline.top_k", 5),
                )
                
                if research_result and research_result.results:
                    research_context = self.research_pipeline.format_research_block(research_result)
                    research_instructions = self.research_pipeline.format_instructions_block(research_result)
                    logger.info(f"Research pipeline: domain={research_result.domain}, "
                               f"results={len(research_result.results)}, "
                               f"conf={research_result.retrieval_confidence:.2f}")
                else:
                    logger.debug("Research pipeline: no results")
            except Exception as e:
                logger.warning(f"Research pipeline error: {e}")
        ...
        voice_context = VoiceContext(
            ...
            research_context=research_context,  # 5th lane: curated knowledge
            research_instructions=research_instructions,  # Domain-specific guidance
        )
